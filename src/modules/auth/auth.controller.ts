...
export const changePassword: RequestHandler = async (req, res) => {
  ...
  res.json(await authService.changePassword(req.auth.userId, body.currentPassword, body.newPassword));
};
export const me: RequestHandler = async (req, res) => {
  if (!req.auth) throw new AppError(401, "AUTH_REQUIRED", "Authentication required");
  const result = await authService.identity(req.auth.userId);
  res.json({
    id: result.user.id,
